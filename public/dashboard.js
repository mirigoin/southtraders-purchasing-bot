...
async function loadBest(){
  ...
  try {
    ...
    tb.innerHTML=d.map(function(q){
      return '<tr><td>'+(q.product||'')+'</td><td>'+(q.model||'')+'</td><td>'+(q.capacity||'')+'</td><td><strong style="color:#68d391"><td>'+(q.qty||'-')+'</td><td>'+(q.incoterm||'')+'</td><td>'+new Date(q.ts).toLocaleDateString()+'</td></tr>';
    }).join('');
...
+q.price+' '+q.currency+'</strong></td><td>'+(q.ultimo_costo!=null?'<span style="color:#a0aec0"><td>'+(q.qty||'-')+'</td><td>'+(q.incoterm||'')+'</td><td>'+new Date(q.ts).toLocaleDateString()+'</td></tr>';
    }).join('');
  } catch(e){ tb.innerHTML='<tr><td colspan="8" style="color:#f87171">Error</td></tr>'; }
}
...
document.addEventListener('DOMContentLoaded', init);
+q.ultimo_costo+'</span>':'<span style="color:#4a5568">-</span>')+'</td><td>'+(q.ultimo_costo!=null?(function(){var d=((q.price-q.ultimo_costo)/q.ultimo_costo*100);var color=d>5?'#fc8181':(d>0?'#f6e05e':'#68d391');return '<span style="color:'+color+'">'+(d>=0?'+':'')+d.toFixed(1)+'%</span>';})():'-')+'</td><td>'+(q.supplier_name||'')+'</td><td>'+(q.qty||'-')+'</td><td>'+(q.incoterm||'')+'</td><td>'+new Date(q.ts).toLocaleDateString()+'</td></tr>';
    }).join('');
  } catch(e){ tb.innerHTML='<tr><td colspan="8" style="color:#f87171">Error</td></tr>'; }
}
