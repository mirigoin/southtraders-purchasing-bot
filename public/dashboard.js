...
function showTab(id,el){
  ...
  if(id==='tab-request'){ loadQuoteRequests(); loadSupplierSelect(); }
...
async function sendRequestQuote(){
  ...
  rows.forEach(function(row){
    var n=row.querySelector(".prod-name");
  ...
  try{
    var body=products.length===1
      ?{product:products[0].name,target_price:products[0].target,supplier_id:supplierId}
      :{products:products,supplier_id:supplierId};
    var r=await fetch("/api/request-quote",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});
    var d=await r.json();
    if(d.ok||d.sent>=0){
...
function loadSupplierSelect(){
  ...
  fetch("/api/suppliers").then(r=>r.json()).then(function(d){
    var list=Array.isArray(d)?d:(d.suppliers||[]);
    list.forEach(function(s){
      var opt=document.createElement("option");
      opt.value=s.id;
      opt.textContent=(s.alias||s.name||"Proveedor "+s.id);
      sel.appendChild(opt);
    });
