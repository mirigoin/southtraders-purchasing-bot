...
app.get('/v2', (req, res) => res.sendFile(path.join(__dirname, 'dashboard-v2.html')));
// ============ ENV ============
...
// Tabla de introducciones: tracking de a quÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ© supplier/grupo ya se presentÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ³ Marco
await pool.query(`
...
// Re-sincronizar secuencias de SERIAL (importante despuÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©s de seed de datos externos)
try {
...
// ============ STOCK-WITHOUT-PRICE DETECTION (Nivel 2 ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ pedir precio) ============
...
async function notifyOwnerStockNoPrice(quotes, msgInfo) {
  ...
  // Rate limit check: descartar productos ya notificados en las ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂºltimas 6h
  ...
  const productLines = noPriceQuotes
    ...
    .map(q => {
      ...
      return `ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ ${desc}${qtyStr}`;
    });

  // Check si Marco ya se presentÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ³ a este supplier. Si no, incluir intro.
  ...
  // Mensaje sugerido en INGLES (sin firma ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ ya saben quiÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©n es)
  // Primera vez: presentaciÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ³n completa. Despues: directo al grano.
  ...
  const alertMsg = [
    `ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Stock without price detected`,
    `From: ${supplierName} (${supplierPhone})`,
    alreadyIntroduced ? '(Marco already introduced to this supplier)' : '(First contact ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ message includes intro)',
    ``,
    `Items:`,
    productLines.join('\n'),
    ``,
...
function findCosto(productSearchStr) {
  ...
  if (!key) return null;
  // 1) Match exacto (la forma mÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¡s confiable)
  ...
  var bestScore = 0;
  var best = null;
  var bestKeyLen = Infinity; // en empate, preferir la entry con MENOS palabras (mÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¡s especÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ­fica al match)
  ...
  for (var k in costosCache.map) {
    ...
    // Score = cuÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¡ntas palabras del search matchearon (siempre = searchWords.length aquÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ­)
...
// TEST: verificar routing de DELETE
app.get('/api/test-delete/:id', (req, res) => {
  res.json({ ok: true, id: req.params.id, method: 'GET-test' });
...
// DELETE quotes via app.route (alternativa robusta)  
app.route('/api/quotes/:id').delete(async (req, res) => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) return res.status(400).json({ error: 'ID invalido' });
  try {
    const result = await pool.query('DELETE FROM quotes WHERE id = $1 RETURNING id', [id]);
...
// GET /api/best-prices - Mejor precio por producto/modelo/capacidad (ÃÂÃÂÃÂÃÂºltimos N dÃÂÃÂÃÂÃÂ­as)
...
// DELETE /api/quotes/:id - Eliminar una cotizaciÃÂÃÂ³n por ID
app.delete('/api/quotes/:id', async (req, res) => {
  ...
  try {
    ...
    console.error('DELETE /api/quotes/:id error:', e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/baileys/logout', async (req, res) => {
  try {
    ...
    baileysClient = null;
    res.json({ ok: true, message: 'Sesion cerrada. LlamÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¡ a /api/baileys/restart para generar QR.' });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/baileys/register', async (req, res) => {
  try {
    ...
    if (!code) {
      ...
      // Paso 2: Confirmar con el cÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ³digo SMS
...
app.post('/api/request-quote', async (req, res) => {
  try {
    ...
    const ownerMsg = sent > 0
      ? 'ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¤ Pedido enviado a ' + sent + ' proveedor(es): ' + supplierNames + '\n' + msg
      : 'ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Pedido registrado (envÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ­o manual requerido):\n' + supplierNames + '\n' + msg;
...
// ============ RECENT MESSAGES (debug ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ incluye media cols) ============
    ...
    for (const msg of change.messages) {
      ...
      if (supplier.rows.length > 0) {
        ...
        if (result.quotes && result.quotes.length > 0) {
          ...
          await alertOwner(`ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ CotizaciÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ³n directa de ${s.name}:\n${summary}`);
        }
      }
    }
  } catch (e) {
...
app.post('/api/admin/delete-quotes', async (req, res) => {
  ...
  if (process.env.BAILEYS_DISABLED === 'true') {
    console.log('[BAILEYS] Disabled via BAILEYS_DISABLED env var ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ skipping init');
  } else {
    await initBaileys();
  }
}
